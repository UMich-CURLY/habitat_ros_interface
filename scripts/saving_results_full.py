...
def save_results(full_result, file_appender):
        ...
        csv_file = folder_name + "demand_plots_time_1000_" + file_appender + ".csv"
...
def main():
    ...
    # Initialize human selections
    got_demand_right = np.array(np.zeros(25), dtype = bool)
    while(not np.all(got_demand_right)):
        for i in range(1,10):
            ...
            if (total_demand_req>25):
                continue
            if (got_demand_right[total_demand_req-1]):
                continue
    ...
    save_results(full_result_greedy, "std_dev_100_greedy_in_time")
    save_results(full_result_greedy_demand, "std_dev_100_greedy_in_demand")
    save_results(full_result, "std_dev_100_optimal")
