...
PARSER.add_argument('-vel', '--vel', default=5, type=int, help='height of the gridworld')
PARSER.add_argument('-omega', '--omega', default=5, type=int, help='width of the gridworld')
...
def generate_demonstrations(gw, policy, n_trajs=100, len_traj=20, rand_start=False, start_pos=[0,0]):
  ...
  for i in range(n_trajs):
    ...
    ### Start the teleop/from bagfiles and initiate environment ###
...
### No Ground truth rewrad maps here ### 
#   rmap_gt = np.zeros([vel, omega])
...
#   values_gt, policy_gt = value_iteration.value_iteration(P_a, rewards_gt, GAMMA, error=0.01, deterministic=True)
  ...
  ### Fidn the transition probability, idk how yet 
  P_a = gw.get_transition_mat()

  # use identity matrix as feature
  feat_map = np.eye(N_STATES)

  ## Why do we need gt policy? 
