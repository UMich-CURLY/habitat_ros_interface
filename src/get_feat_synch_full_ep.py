...
GRID_SIZE_IN_M = 6
METRIC_CSV = "/habitat-lab/data/data_metrics/Jul_9/metrics_data.csv"
METRIC_CSV = "/habitat-lab/data/vids/Train_data_15/HUMAN_ORCA/results.csv"
...
OUT_DIR = "/home/catkin_ws/src/habitat_ros_interface/data/dataset_full_ep/irl_jul_24_2/"
OUT_DIR = "/home/catkin_ws/src/habitat_ros_interface/data/dataset_full_ep/irl_sept_24_3/"
...
def get_dist_from_door(current_pos, door_pos):
    ...
    try:
        val = np.linalg.norm(np.cross(p2-p1, p1-p3))/np.linalg.norm(p2-p1)
    except:
        val = 0
    return val
...
class FeatureExpect():
    ...
    def grid_to_pix(self, point_2d):
        ...
        if point[0] >=self.grid_dimension:
            point[0] = self.grid_dimension-1
        if point[1] >=self.grid_dimension:
            point[1] = self.grid_dimension-1
        return point
    ...
    def people_callback(self,msg):
        ...
        a.header = msg.header
        a.pose = msg.poses[0]
        human_pose_msg = transform_pose(self.transformation, a)
        orientation = [human_pose_msg.orientation.x, human_pose_msg.orientation.y, human_pose_msg.orientation.z, human_pose_msg.orientation.w]
        self.human_angle = -euler_from_quaternion(orientation)[2]
        self.human_pose_msg = PoseStamped()
        self.human_pose_msg.pose = human_pose_msg
    ...
    def save_feature(self):
        ...
        with open(self.full_path + "/ep_count.txt", 'w') as f:
            ...
            f.close()
        with open(self.full_path + "/crossing_count.txt", 'w') as f:
            f.write(str(self.crossing_counter))
            f.close()
        try:
        ...
        if (not (len(entry) == 0)):
            ...
            __ = os.system("mkdir " + next_folder_name)
        print ("new folder is , continue?", next_folder_name)
        self.im_array[0].save(self.full_path + "/episode_" + str(self.ep_counter) + ".gif", save_all=True, append_images=self.im_array[1:], duration=100, loop=0)
        self.im_array = []
    ...
    def get_current_feature(self):
        ...
        if len(self.human_past_traj) > 2 and len(self.robot_past_traj) > 2:
            ...
            if dist_betweet_robot > 30 or dist_between_human > 30:
        ...
        if (self.robot_pose_2d) is not None:
                # self.overlayed_grid_img[self.robot_pose_2d[1], self.robot_pose_2d[0]] = [255,0,0]
                self.robot_past_traj.append(self.robot_pose_2d)
                if len(self.robot_past_traj) == 1 and len(self.robot_vel) == 0:
                    self.robot_vel.append(0)
                if len(self.robot_past_traj) > 2:
                    print("Time between poses is ", time_between_poses)
                    self.robot_vel.append(np.linalg.norm(np.array(self.robot_past_traj[-1])-np.array(self.robot_past_traj[-2]))*GRID_RESOLUTION/time_between_poses)
        else:
            print("robot not in frame anymore", self.robot_pose_2d)
            pass
        ...
        #             return
        if len(self.human_past_traj) <2:
            print("Exiting because human past less than 2")
            mutex.release()
            return
        ...
        if (self.grid_img == np.zeros(self.grid_img.shape)).all():
            ...
            print("Exiting because grid img is zero")
            mutex.release()
            return
        ...
        # caption_area = 255*np.ones((20,60,3))
        # img_array = np.append(self.new_overlayed_grid_image, caption_area, axis = 0)
        # im = img.fromarray(np.uint8(img_array))
        im = img.fromarray(np.uint8(self.new_overlayed_grid_image))
        # font = ImageFont.load_default()
        # w,h = font.getsize("episode"+" "+str(self.counter))
        # draw = ImageDraw.Draw(im)
        # draw.text((0,60), "episode"+" "+str(self.counter), (255,255,255), font=font)
        ...
        with open(folder_path+ "/vels.npy", 'wb') as f:
            np.save(f, np.array([self.robot_vel[-1], self.human_vel[-1]]))
        ...
        self.counter += 1
        print("Should have updated counter to!! ", self.counter)
        mutex.release()
...
if __name__ == "__main__":
        ...
        while(not rospy.is_shutdown()):
            ...
            if feature.reset:
                ...
                __ = os.system("rosrun habitat_interface get_feat_synch_full_ep.py")
            ...
            if (human_cleared) and not one_cleared:
                print("Human has cleared")
                feature.crossing_counter = feature.counter
                one_cleared = True
            # if (human_cleared and not one_cleared):
